...
export function getStyle(element, ancestors, previousStyle = {}) {
  ...
  const color = elAttributes.mathcolor ||
    ancestors.find((element) =>
      element.name === "mstyle" && element.attribs &&
      element.attribs.color
    )?.attribs.color || "";
  ...
  const size = elAttributes.mathsize ||
    ancestors.find((element) =>
      element.name === "mstyle" && element.attribs &&
      element.attribs.mathsize
    )?.attribs.mathsize || "";
  const scriptlevel = elAttributes.scriptlevel ||
    ancestors.find((element) =>
      element.name === "mstyle" && element.attribs &&
      element.attribs.scriptlevel
    )?.attribs.scriptlevel || "";
  const background = elAttributes.mathbackground ||
    ancestors.find((element) =>
      element.name === "mstyle" && element.attribs &&
      element.attribs.mathbackground
    )?.attribs.mathbackground || "";
  let variant = elAttributes.mathvariant ||
    ancestors.find((element) =>
      element.name === "mstyle" && element.attribs &&
      element.attribs.mathvariant
    )?.attribs.mathvariant || "";
  if (variant === "b-i") {
    variant = "bold-italic";
  }
  const fontweight = elAttributes.fontweight ||
    ancestors.find((element) =>
      element.name === "mstyle" && element.attribs &&
      element.attribs.fontweight
    )?.attribs.fontweight || "";
  if (fontweight === "bold" && !["bold", "bold-italic"].includes(variant)) {
    ...
    fontweight === "normal" && ["bold", "bold-italic"].includes(variant)
  ) {
    if (variant.includes("italic")) {
      variant = "italic";
    } else {
  ...
  const fontstyle = elAttributes.fontstyle ||
    ancestors.find((element) =>
      element.name === "mstyle" && element.attribs &&
      element.attribs.fontstyle
    )?.attribs.fontstyle || "";
  if (
    fontstyle === "italic" && !["italic", "bold-italic"].includes(variant)
  ) {
    ...
    fontstyle === "normal" && ["italic", "bold-italic"].includes(variant)
  ) {
    if (variant.includes("bold")) {
      variant = "bold";
    } else {
