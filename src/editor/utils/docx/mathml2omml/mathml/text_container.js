...
function textContainer(
  ...
  if (previousSibling.isNary) {
    const previousSiblingTarget =
      targetParent.children[targetParent.children.length - 1];
    targetParent = previousSiblingTarget
      .children[previousSiblingTarget.children.length - 1];
  }

  const hasMglyphChild = element.children &&
    element.children.find((element) => element.name === "mglyph");
  ...
  const sameGroup = ( // Only group mtexts or mi, mn, mo with oneanother.
    ...
    ["mi", "mn", "mo"].includes(textType) &&
    ["mi", "mn", "mo"].includes(previousSibling?.name)
  );
  let targetElement;
