...
export function $convertTextNode(node: TextNode) {
  ...
  const tableCellColor = nearestTableCell
    ? $getNodeStyleValueForProperty(nearestTableCell, "color").replace(
      "inherit",
      "",
    )
    : undefined;
  const fontsizeInPx = parseInt(
    $getNodeStyleValueForProperty(node, "font-size"),
  );
  const color =
    $getNodeStyleValueForProperty(node, "color").replace("inherit", "") ||
    tableCellColor;
  const backgroundColor =
    $getNodeStyleValueForProperty(node, "background-color").replace(
      "inherit",
      "",
    ) || undefined;
  ...
  const textRun = new TextRun({
    ...
    font: node.hasFormat("code")
      ? "Consolas"
      : $getNodeStyleValueForProperty(node, "font-family"),
