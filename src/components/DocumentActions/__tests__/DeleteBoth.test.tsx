...
describe('DeleteBothDocument', () => {
  ...
  test('renders as menu item when variant is menuitem', () => {
    render(<DeleteBothDocument userDocument={sampleDocument} variant="menuitem" />);
  ...
  test('handles delete action for both local and cloud', async () => {
    ...
    const mockAlert = jest.fn().mockResolvedValue({ payload: 'test-delete-id' });
    const mockDeleteLocal = jest.fn();
    const mockDeleteCloud = jest.fn();
    
    (actions.alert as jest.Mock).mockImplementation(mockAlert);
    (actions.deleteLocalDocument as jest.Mock).mockImplementation(mockDeleteLocal);
    (actions.deleteCloudDocument as jest.Mock).mockImplementation(mockDeleteCloud);
