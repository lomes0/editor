...
const MoveToDialog: React.FC<MoveToDialogProps> = ({ open, onClose, userDocument }) => {
  ...
  const loadDirectories = (directoryId: string | null) => {
    try {
      ...
      const buildBreadcrumbs = (dirId: string | null, trail: { id: string | null; name: string; }[] = []) => {
        if (dirId === null) {
  ...
  const isChildOf = (directoryId: string, potentialParentId: string): boolean => {
    try {
      ...
      const checkChildWithDepth = (dirId: string, potParentId: string, depth: number): boolean => {
        if (depth >= MAX_RECURSION_DEPTH) {
          console.warn('Max recursion depth reached in isChildOf check');
          return false;
        }
  ...
  return (
    ...
    >
      <DialogTitle id="move-dialog-title">
        Move {documentType === DocumentType.DIRECTORY ? 'Folder' : 'Document'}: {documentName}
      </DialogTitle>
      ...
      <DialogContent dividers>
          ...
          {loading ? (
            ...
            <List>
              {directories.map((directory) => {
                const dirName = directory.local?.name || directory.cloud?.name || 'Directory';
                ...
                return (
                  <ListItem key={directory.id} disablePadding>
                    ...
                    >
                      <ListItemIcon>
                        <Folder color={isCurrentParent ? "primary" : "inherit"} />
                      </ListItemIcon>
                      <ListItemText 
                        primary={dirName} 
            ...
            <Box sx={{ mt: 2 }}>
              <Button
                ...
                onClick={() => {
                  ...
                  if (parentIndex >= 0) {
                    handleBreadcrumbClick(breadcrumbs[parentIndex].id);
                  }
                }}
                size="small"
              >
        ...
        <Box>
          <Typography variant="body2" color="text.secondary">
            Current location: {
              currentParentId 
                ? documents.find(doc => doc.id === currentParentId)?.local?.name || 
                  documents.find(doc => doc.id === currentParentId)?.cloud?.name || 
                  'Unknown'
                : 'Root'
            }
          </Typography>
          <Typography variant="body2" color="text.secondary">
            New location: {
              currentDirectoryId 
                ? documents.find(doc => doc.id === currentDirectoryId)?.local?.name || 
                  documents.find(doc => doc.id === currentDirectoryId)?.cloud?.name || 
                  'Unknown'
                : 'Root'
            }
          </Typography>
      ...
      <DialogActions>
        <Button onClick={onClose} data-testid="move-cancel-button">Cancel</Button>
        <Button 
          onClick={handleMove} 
          variant="contained"
          color="primary"
