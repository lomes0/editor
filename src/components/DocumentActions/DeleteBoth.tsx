...
const DeleteBothDocument: React.FC<{ 
  ...
  const handleDelete = async () => {
    ...
    const alert = {
      ...
      content: `Are you sure you want to delete ${isDirectory ? `folder "${name}"` : `document "${name}"`}? This will remove it from both cloud and local storage.`,
      actions: [
  ...
  return <IconButton aria-label={`Delete ${itemType}`} onClick={handleDelete} size="small"><DeleteForever /></IconButton>
}
