...
import { 
  Badge, Chip, IconButton, Skeleton, Avatar, Card, 
  Typography, Box, Divider, CardContent,
  CardActionArea
} from '@mui/material';
...
const DirectoryCard: React.FC<{ userDocument?: UserDocument, user?: User, sx?: SxProps<Theme> }> = memo(({ userDocument, user, sx }) => {
  ...
  const statusChip = () => {
    if (!userDocument) return <Skeleton variant="rounded" width={60} height={24} />;
    if (isLocalOnly) return <Chip size="small" variant="outlined" icon={<MobileFriendly />} label="Local" />;
    if (isUploaded) return <Chip size="small" variant="outlined" icon={isUpToDate ? <CloudDone /> : <CloudSync />} label={isUpToDate ? "Synced" : "Out of Sync"} />;
    if (isCloudOnly) return <Chip size="small" variant="outlined" icon={<Cloud />} label="Cloud" />;
    return null;
  };

  // Author chip
  const authorChip = () => {
    if (!document) return <Skeleton variant="rounded" width={100} height={24} />;
    return (
      <Chip
        size="small"
        variant="outlined"
        avatar={<Avatar alt={author?.name ?? "Local User"} src={author?.image ?? undefined} />}
