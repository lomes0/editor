...
export default class MatchFinder {
  ...
  normalizeForIndex(word: string) {
    ...
    if (
      this.options.ignoreWhitespaceDifferences && Utils.isWhiteSpace(word)
    ) {
      return " ";
    }
  ...
  findMatch() {
    ...
    for (
      ...
      for (let indexInNew of this.wordIndices.get(index)!) {
        let newMatchLength = (matchLengthAt.has(indexInNew - 1)
          ? matchLengthAt.get(indexInNew - 1)
          : 0) + 1;
        newMatchLengthAt.set(indexInNew, newMatchLength);

        if (newMatchLength > bestMatchSize) {
          bestMatchInOld = indexInOld - newMatchLength - blockSize +
            2;
          bestMatchInNew = indexInNew - newMatchLength - blockSize +
            2;
  ...
  removeRepeatingWords() {
    let threshold = this.newWords.length +
      this.options.repeatingWordsAccuracy;
