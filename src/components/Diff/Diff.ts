...
class HtmlDiff {
  ...
  constructor(
    oldText: string,
    newText: string,
    options?: Partial<HTMLDiffOptions>,
  ) {
    this.content = [];
    ...
    this.ignoreWhiteSpaceDifferences = options?.ignoreWhiteSpaceDifferences ??
      false;
  ...
  processInsertOperation(opp: Operation, cssClass: string) {
    let text = this.newWords.filter((_, pos) =>
      pos >= opp.startInNew && pos < opp.endInNew
    );
    this.insertTag("ins", cssClass, text);
  }

  processDeleteOperation(opp: Operation, cssClass: string) {
    let text = this.oldWords.filter((_, pos) =>
      pos >= opp.startInOld && pos < opp.endInOld
    );
    this.insertTag("del", cssClass, text);
  }

  processEqualOperation(opp: Operation) {
    let result = this.newWords.filter((_, pos) =>
      pos >= opp.startInNew && pos < opp.endInNew
    );
    ...
    while (words.length) {
      ...
      if (nonTags.length !== 0) {
        ...
        if (specialCaseOpeningTagRegex.test(words[0])) {
          ...
          let openingTag = this.specialTagDiffStack.length === 0
            ? null
        ...
        if (
          words.length === 0 && specialCaseTagInjection.length === 0
        ) {
          break;
        }
        ...
        if (specialCaseTagInjectionIsBefore) {
          this.content.push(
            specialCaseTagInjection +
              this.extractConsecutiveWords(words, Utils.isTag)
                .join(""),
          );
        } else {
          this.content.push(
  ...
  *removeOrphans(matches: Match[]) {
    ...
    for (let next of matches) {
      ...
      let oldDistanceInChars = this.oldWords.slice(
        prev?.endInOld,
        next.startInOld,
      ).reduce(sumLength, 0);
      let newDistanceInChars = this.newWords.slice(
        prev?.endInNew,
        next.startInNew,
      ).reduce(sumLength, 0);
      let currMatchLengthInChars = this.newWords.slice(
        curr.startInNew,
        curr.endInNew,
      ).reduce(sumLength, 0);
      if (
  ...
  findMatchingBlocks(
    ...
    if (match !== null) {
      if (
        startInOld < match.startInOld && startInNew < match.startInNew
      ) {
        this.findMatchingBlocks(
          startInOld,
          match.startInOld,
